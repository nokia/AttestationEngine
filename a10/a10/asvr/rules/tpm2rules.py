...
class PCRsAllUnassigned(baserule.BaseRule):
	...
	def apply(self):
		try:
			...
			return self.returnMessage(a10.structures.constants.VERIFYERROR, "Missing bank parameter. Was expecting sha1, sha256, sha384 or sha512", [])

		if bank not in ["sha1","sha256","sha384","sha512","test"]:
			return self.returnMessage(a10.structures.constants.VERIFYERROR, "Unknown PCR bank, got "+bank+" was expecting sha1, sha256, sha384 or sha512", [])

		try:
			pcrs = self.claim["payload"]["payload"]["pcrs"][bank]
		except KeyError:
			return self.returnMessage(a10.structures.constants.VERIFYERROR, "PCR bank "+bank+" not supported on this TPM", [])
		...
		if trusted==True:
			return self.returnMessage(a10.structures.constants.VERIFYSUCCEED, "All PCRs unassigned", [])
		else:
...
class TPM2FirmwareVersion(baserule.BaseRule):
	...
	def apply(self):
		...
		try:
			...
			claimedAttestedValue = str(self.claim["payload"]["payload"]["quote"]["firmwareVersion"])
		...
		if trusted == True:
			return self.returnMessage(a10.structures.constants.VERIFYSUCCEED, "attested value == known good value", [])
		else:
...
class TPM2QuoteMagicNumber(baserule.BaseRule):
	...
	def apply(self):
		try:
			...
			return self.returnMessage(a10.structures.constants.VERIFYERROR, "Missing magic number. Does this contain a TPMS_ATTEST structure?", [])

		if trusted == True:
			return self.returnMessage(a10.structures.constants.VERIFYSUCCEED, "magic == ff544347", [])
		else:
			return self.returnMessage(a10.structures.constants.VERIFYFAIL, "Incorred magic number", [])
...
class TPM2QuoteType(baserule.BaseRule):
	...
	def apply(self):
		try:
			...
			return self.returnMessage(a10.structures.constants.VERIFYERROR, "Missing type. Does this contain a TPMS_ATTEST structure?", [])

		if trusted == True:
			return self.returnMessage(a10.structures.constants.VERIFYSUCCEED, "type == 801", [])
		else:
			return self.returnMessage(a10.structures.constants.VERIFYFAIL, "Incorred TPMS_ATTEST type", [])
...
class TPM2QuoteAttestedValue(baserule.BaseRule):
	...
	def apply(self):
		...
		try:
			...
			return self.returnMessage(a10.structures.constants.VERIFYERROR, "Missing pcrDigest value in expected value. ", [])		


		try:	
			claimedAttestedValue = self.claim["payload"]["payload"]["quote"]["attested"]["quote"]["pcrDigest"]
		except KeyError:
			return self.returnMessage(a10.structures.constants.VERIFYERROR, "Missing pcrDigest in quote. Is this a valid TPMS_ATTEST structure?", [])		
...
class TPM2Safe(baserule.BaseRule):
	...
	def apply(self):
		try:
			trusted = (self.claim["payload"]["payload"]["quote"]["clockInfo"]["safe"] == 1)
		except KeyError:
			return self.returnMessage(a10.structures.constants.VERIFYERROR, "Missing safe value in quote. Is this a valid TPMS_ATTEST structure?", [])		


		if trusted == True:
			return self.returnMessage(a10.structures.constants.VERIFYSUCCEED, "Safe", [])
		else:
...
class TPM2QuoteStandardVerify(baserule.BaseRule):
	...
	def apply(self):
		...
		if (magicRule_result['result'] == a10.structures.constants.VERIFYNORESULT) or (
					quoteType_result['result'] == a10.structures.constants.VERIFYNORESULT) or (
							  safe_result['result'] == a10.structures.constants.VERIFYNORESULT) or (
							  av_result['result'] == a10.structures.constants.VERIFYNORESULT) or (
							  fw_result['result'] == a10.structures.constants.VERIFYNORESULT):
			return self.returnMessage(a10.structures.constants.VERIFYNORESULT, "Subrule failed - see additional section", subresults)

		# Check if anything failed = ERROR
		# For some other reason

		if (magicRule_result['result'] == a10.structures.constants.VERIFYERROR) or (
					quoteType_result['result'] == a10.structures.constants.VERIFYERROR) or (
							  safe_result['result'] == a10.structures.constants.VERIFYERROR) or (
							  av_result['result'] == a10.structures.constants.VERIFYERROR) or (
							  fw_result['result'] == a10.structures.constants.VERIFYERROR):
			return self.returnMessage(a10.structures.constants.VERIFYERROR, "Subrule failed - see additional section", subresults)

		# Ok, now check if everything went well or not, ie: trusted yay or nay!

		trusted = (magicRule_result['result'] == a10.structures.constants.VERIFYSUCCEED) and (
					quoteType_result['result'] == a10.structures.constants.VERIFYSUCCEED) and (
							  safe_result['result'] == a10.structures.constants.VERIFYSUCCEED) and (
							  av_result['result'] == a10.structures.constants.VERIFYSUCCEED) and (
							  fw_result['result'] == a10.structures.constants.VERIFYSUCCEED)

		msg = "Additional contains " + str(len(subresults)) + " items. Expected value in subrule - see additional section"

		if trusted == True:
			return self.returnMessage(a10.structures.constants.VERIFYSUCCEED, msg, subresults)
		else:
			return self.returnMessage(a10.structures.constants.VERIFYFAIL, msg, subresults)

...
def getDatabaseStatus():
	...
	for c in ['elements', 'policies', 'expectedvalues', 'claims', 'results', 'hashes', 'log']:
		collection = asdb[c]
...
def getClaimsFull(n):
	...
	collection = asdb["claims"]
	e = collection.find({}, {'_id': False }).sort("header.as_requested", pymongo.DESCENDING).limit(n)
	return list(e)
...
def getAssociatedResults(i):
	...
	rs = collection.find({"claimID":i }, {'_id': False }).sort("verifiedAt", pymongo.DESCENDING)
	return list(rs)
...
def getResultsFull(n):
	...
	collection = asdb["results"]
	e = collection.find({}, {'_id': False }).sort("verifiedAt", pymongo.DESCENDING).limit(n)
	return list(e)    
...
def getLatestResults(e,n):
	...
	collection = asdb["results"]
	rs = list(collection.find({"elementID": e}).sort("verifiedAt", pymongo.DESCENDING).limit(n))
	return rs    
...
def getLatestResultsForElementAndPolicy(e,p,n):
	...
	collection = asdb["results"]
	rs = list(collection.find({"elementID": e, "policyID":p}).sort("verifiedAt", pymongo.DESCENDING).limit(n))
	return rs        
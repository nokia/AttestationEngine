...
def main(cert, key, config_filename='ta_config.cfg'):
    ...
    if cert and key:
        ta.run(debug=ta.config['FLASKDEBUG'], threaded=ta.config['FLASKTHREADED'], host=ta.config['DEFAULTHOST'],
                          port=ta.config['DEFAULTPORT'], ssl_context=(cert, key))
    else:
        ta.run(debug=ta.config['FLASKDEBUG'], threaded=ta.config['FLASKTHREADED'], host=ta.config['DEFAULTHOST'],

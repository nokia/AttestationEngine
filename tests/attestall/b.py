...
def printResult(v):
    r = ast.literal_eval(requests.get(asvr+"/result/"+v).text)
    print("R=",r["ruleName"],r["message"],r["result"])

def processVerify(k):
    # k is a claim rule structure
    vr = requests.post(asvr+"/verify", json=k).text
    #print("V ",k,vr)
    thr = threading.Thread(target=printResult, args=(vr,))
    thr.start()
...
def attestByType(t,p,rs):
    ...
    for e in es:
        ...
        print("  ... attesting ",el["name"],"by",pl["name"])
        thr.start()

...
attestation_blueprint = Blueprint('attestation', __name__, static_folder='../static', template_folder='../templates/')
...
def attestverify_post():
	...
	if  cres.rc() != a10.structures.constants.SUCCESS:
		flash("Error obtaining claim: " + str(cres.msg())+" "+str(cres.rc()), 'danger')
		return redirect("/attestverify/"+i)
	else:
		message = Markup("Claim <a href=/claim/"+str(cres.msg())+">"+str(cres.msg())+"</a> successfully obtained")
		flash(message,"success")
		if av=="aonly":
			flash("NB: request was for attestation only (no verification applied)", 'info')
			...
			if v.rc() != a10.structures.constants.RESULTSUCCESSFUL:
				...
				message = Markup("Result <a href=/result/"+str(v.msg())+">"+str(v.msg())+"</a> successfully obtained")
				flash(message,"success")
...
def attestverifyall_post():
	...
	flash("Attesting "+str(lev)+" elements with session string "+sessionstring, 'primary')

	for a in attreqs:
		print("Attesting ",a["policyid"],a['op'])

		if a['op']=='d':
			flash("NB: attest/verify for " + a["policyid"] +" was not required", 'secondary')
		else:    
			...
			if  cres.rc() != a10.structures.constants.SUCCESS:
				...
				flash(message,"success")
				if a['op']=="a":
					flash("NB: claim " + str(cres.msg())+" was for attestation only (no verification applied)", 'info')
				else:

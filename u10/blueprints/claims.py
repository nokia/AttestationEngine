...
claims_blueprint = Blueprint('claims', __name__, static_folder='../static', template_folder='../templates/')
...
def claims():
	...
	for c in cs:
		...
		c['timedifference'] = round(float(c['header']['as_received']) - float(c['header']['as_requested']),4)
...
def claim(item_id):
	...
	for r in rs:
		r['difference'] = round(float(r['verifiedAt']) - float(c['header']['as_received']),4)

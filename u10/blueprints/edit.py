...
edit_blueprint = Blueprint('edit', __name__, static_folder='../static', template_folder='../templates/')
...
def post_edited_element():
    ...
    # print("J",type(j),j,"\n","I",i,"\n")
    if j['itemid'] != i:
        flash("Attempt to modify itemid in raw JSON form ignored - do not try to do this again OK!", 'warning')
        j['itemid'] = i

    r = a10.asvr.elements.updateElement(j)
    # print("Return code ",r)
    if r.rc() != a10.structures.constants.SUCCESS:
        flash("Database update error code " + str(r.rc()) + " " + str(r.msg()), 'danger')
    else:
...
def new_element():
    # print("new element")
    element_data = {"type": ["tpm2.0"], "name": "EDIT_THIS", "description": "EDIT_THIS_LONGER_DESCRIPTION",
                   "endpoint": "http://127.0.0.1:8530", "protocol": "A10HTTPREST"}
...
def new_policy():
    # print("new element")
    element_data = {"intent": "tpm2/quote", "name": "EDIT_THIS", "description": "EDIT_THIS_LONGER_DESCRIPTION",
...
def edit_expected_value(item_id):
    ...
    if element_data.rc()!=0:
        flash("Element does not exist " + itemid+" code is "+element_data.rc(), 'danger')
    else:
        pp = json.dumps(element_data.msg(), sort_keys=True, indent=4)
        return render_template("editraw.html", t="expectedvalue", e=element_data.msg(), pp=pp)

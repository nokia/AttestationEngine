...
elementanalytics_blueprint = Blueprint('elementanalytics', __name__, static_folder='../static', template_folder='../templates/')
...
def eapage(item_id):
	...
	ds = []
	for p in ps:
		entry = { "elementID": item_id,
				  "elementname": e['name'],
				  "policyID": p['itemid'],
				  "policyname":p['name']
				 }

		counts = a10.asvr.analytics.elementanalytics.getResultCountsByPolicy(item_id,p['itemid'],250)
		#zips are clever!
		...
		entry['vname']= "".join(random.choice(string.ascii_letters) for x in range(random.randint(10, 15)))
		ds.append(entry)
